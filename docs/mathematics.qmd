...
$$
\begin{split}
\mathcal Q(\vec \theta; \vec \theta') = \mathbb E_{\vec Z_t\sim p(Z \mid \vec\alpha_t,\vec\theta)}[\log p_{\vec\theta}(Z^{(T)}, A^{(T)})\mid Z^{(T)}],
\end{split}
$$ {#eq-Qdef}

where $Z^{(T)} = \{\vec Z_t\}_{t=0,\dots,T}$, $A^{(T)} = \{\vec \alpha_t\}_{t=0,\dots,T}$ and $A^{(T-1)} = \{\vec \alpha_t\}_{t=0,\dots,T-1}$. This approximates $\log p_\theta(\{\vec Z_t\}, \{\vec\alpha_t\})$.
Using our model hierarchy, we have, 

$$
\begin{split}
p_{\vec\theta}(Z^{(T)}, A^{(T)}\mid \vec\theta) &= p(Z^{(T)}\mid A^{(T-1)}, \vec\theta)\\
&\times p(A^{(T)} \mid A^{(T-1)}, \vec\theta)\\
&\times p(\alpha_0\mid \vec\theta).
\end{split}
$$

We will tackle each of these terms individually. Firstly, for the data generating term, we have (using $\eqc$ to denote 'equal up to an additive constant')

$$
\begin{split}
&-2\log p(Z^{(T)}\mid A^{(T)}, \vec\theta)\\ 
&\eqc \sum_{t=0}^T \left(\log(\det(\Sigma_\epsilon)) + (\vec Z(t) - \Phi\vec\alpha(t) - X\vec\beta)^\intercal \Sigma_\epsilon^{-1}(\vec Z(t) - \Phi\vec\alpha(t) - X\vec\beta)\right)\\
&=T\log\det(\Sigma_\epsilon) + \mathrm{tr}(\Sigma_\epsilon^{-1}\{\left[\sum_{t=0}^T\vec Z(t)\vec Z(t)^\intercal\right] - 2\Phi\left[\sum_{t=0}^T\vec\alpha(t)\vec Z(t)^\intercal\right]\\
...
$$
\begin{split}
&-2\log p(A^{(T)}mid A^{(T-1)}, \vec\theta)\\ 
&\eqc\sum_{t=1}^T \left(\log(\det(\Sigma_\eta))+(\vec \alpha(t) - M(\vec\theta_\kappa)\alpha(t-1))^\intercal\Sigma_\eta^{-1}(\vec \alpha(t) - M(\vec\theta_\kappa)\alpha(t-1))\right)\\
&= T\log(\det(\Sigma_\eta)) + \mathrm{tr}(\Sigma_\eta^{-1}\{\left[\sum_{t=1}^T\vec\alpha(t)\vec\alpha(t)^\intercal\right] - 2M(\vec\theta_\kappa) \left[\sum_{t=1}^T\vec\alpha(t-1)\vec\alpha(t)^\intercal\right] \\
...
&\eqc \log(\det(\Sigma_0)) + (\vec\alpha_0 - \vec m_0)^\intercal \Sigma_0^{-1} (\vec\alpha_0 - \vec m_0)\\
&= \log(\det(\Sigma_0)) + \mathrm{tr}(\Sigma_0^{-1}\{\vec\alpha_0\vec\alpha_0^\intercal\} - 2\vec\alpha_0\vec m_0^\intercal + \vec m_0\vec m_0^\intercal).
\end{split}
$$

Taking the expectation, summing, and giving names to some relevant quantities,

$$
\begin{split}
\mathcal Q(\vec\theta;\vec\theta') &= \mathbb E_{Z^{(T)}\sim p(Z \mid A^{(T)},\vec\theta')}[\log p_{\vec\theta}(Z^{(T)}, A^{(T)}\mid Z^{(T)} = z^{(T)}]\\
&\eqc -\frac12(\mathrm{tr}(\Sigma_\epsilon^{-1}\{\Xi^{(z)} - 2\Phi\Xi^{(\alpha,z)}- 2X\vec\beta\xi^{(z)\intercal} + \Phi\Xi^{(\alpha)}\Phi^\intercal\\
...
&\quad\quad + \log(\det(\Sigma_\epsilon^T\Sigma_\eta^T\Sigma_0)))
\end{split} 
$${#eq-Q}
...
\xi^{(\alpha)} &:= \sum_{t=0}^T\mathbb{E}_{\vec\theta'}[\vec \alpha(t)\mid Z^{(T)} = z^{(T)}]\\
\Xi^{(z)} &:= \sum_{t=0}^T\vec z(t)\vec z(t)^\intercal\\
\Xi^{(\alpha,z)} &:= \sum_{t=0}^T\mathbb{E}_{\vec\theta'}[\vec\alpha(t)\mid Z^{(T)} = z^{(T)}]\vec z(t)^\intercal\\
\Xi^{(\alpha)} &:= \sum_{t=0}^T\mathbb{E}_{\vec\theta'}[\vec\alpha(t)\vec\alpha(t)^\intercal\mid Z^{(T)} = z^{(T)}]\\
\Xi^{(\alpha,-1)} &:= \sum_{t=1}^T\mathbb{E}_{\vec\theta'}[\vec\alpha(t-1)\vec\alpha(t)^\intercal\mid Z^{(T)} = z^{(T)}]
\end{split}
$$ {#eq-kalmanterms}

In the EM algorithm, an estimate for the Maximum Likelihood estimate involves changing $\theta$ in order to increase (@eq-Q), which can be shown to guarantee that the Likelihood $L(\vec\theta)$ also increases.
The idea is then that repeatedly alternating between adjusting $\vec\theta$ to increase @eq-Q, and then recomputing the terms in @eq-kalmanterms.
These terms are computed using standard Kalman techniques.
These give optimal linear estimates of these quantities (and the optimal overall estimates for the Gaussian case). 
A description of this method is presented in the next section.
...
These are computed in two phases; filtering to find $\{m_{t\mid t}\}_{t=0,\dots,T}$ and $\{P_{t\mid t}\}_{t=0,\dots,T}$, then a smoothing phase to find $\{m_{t\mid T}\}_{t=0,\dots,T}$ and $\{P_{t\mid T}\}_{t=0,\dots,T}$, which are exactly the values needed for @eq-kalmanterms.

To move the filter forward, that is, given $m_{r\mid s}$ and $P_{r\mid s}$, to get $m_{t+1\mid t+1}$ and $P_{t+1\mid t+1}$, we first do
$$
\begin{split}
\vec m_{t+1\mid t} &= M \vec x_{t\mid t}\\
P_{t+1\mid t} &= M P_{t\mid t} M^\intercal + \Sigma_\eta,
\end{split}
$$ {#eq-kalman-midterms}

then we add our new information, $z_{t-1}$, adjusted for the _Kalman gain_;
$$
\begin{split}
m_{t+1\mid t+1} &= \vec m_{t+1\mid t} + K_t(z_t-\Phi \vec m_{t+1\mid t}-X\vec\beta)\\
P_{t+1\mid t+1} &= [I- K_t\Phi]P_{t+1\mid t}
\end{split} 
$$ {#eq-kalmanfilt}
where $K_t$ is the Kalman gain;
$$
\begin{split}
K_t = P_{t+1\mid t}\Phi^\intercal [\Phi P_{t+1\mid t} \Phi^\intercal + \Sigma_\epsilon]^{-1}.
\end{split}
$$
...
We can then work backwards from these values using what is known as the _Kalman Smoother_;
$$
\begin{split}
\vec m_{t-1\mid T} &= \vec m_{t-1\mid t-1} + J_{t11}(\vec m_{t\mid T} - \vec m_{t\mid t-1}),\\
...
We can then also use the _Lag-One Covariance Smoother_.
From 
$$
\begin{split}
P_{T,T-1\mid T} = (I - K_T\Phi) MP_{T-1\mid T-1},
\end{split}
$$
...
\end{split}
$$ {#eq-lag1smooth}
Then, to find the expectations we need in @eq-kalmanterms, we have;

$$
\begin{split}
\mathbb E_{\theta'}[\vec\alpha(t)\mid Z^{(T)}=z^{(T)}] &= \vec m_{t \mid T}\\
\mathbb E_{\theta'}[\vec\alpha(t)\vec\alpha(t)^\intercal\mid Z^{(T)}=z^{(T)}] &= P_{t\mid T} - \vec m_{t\mid T}\vec m_{t\mid T}^\intercal\\
\mathbb E_{\theta'}[\vec\alpha(t-1)\vec\alpha(t)^\intercal\mid Z^{(T)}=z^{(T)}] &= P_{t,t-1\mid T}^\intercal - \vec m_{t\mid T} \vec m_{t-1\mid T}^\intercal.
\end{split}
$$
...
<span style="color: red;">[NOTE: Of course, we could also just maximise the data likelihood computed the the Kalman filter (see p 304 in the above book). In this book, it is claimed that this performs comparitively better, but in this case we are dealing with much higher dimensions.
The next step is to implement both of these and compare the two.
]</span>
