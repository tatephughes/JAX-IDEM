...
$$\begin{split}
Z_t(\vec s) &= Y_t(\vec s) + X(\vec s)^{\intercal}\vec \beta + \epsilon_t(\vec s)\\
Y_{t+1}(\vec s) &= \int_{\mathcal D_s} \kappa(s,r) Y_t(r) d\vec r + \omega_t(\vec s). 
\end{split}
$${#eq-IDEM}

Where $\omega_t(\vec s)$ is a small scale gaussian variation with no temporal dynamics [@cressie2015statistics call this a 'spatially descriptive' component], $\vec X(\vec s)$ are spatially varying covariates (for example, in a large-scale climate scenario, this might simply be latitude), $Z$ is observed data, $Y$ is an unobserved dynamic process, $\kappa$ is the driving 'kernel' function, and $\epsilon_t$ is a gaussian white noise 'measurement error' term.

# Process Decomposition

In order to work with the process, we likely want to consider the spectral decomposition of it. 
That is, choose a complete class of spatial spectral basis functions, $\phi_i(\vec s)$, and decompose;

$$\begin{split}
Y_t(\vec s) \approx \sum_{i=1}^{r} \alpha_{i,t} \phi_i(\vec s).
\end{split}
$${#eq-processdecomp}
...
$$\begin{split}
\vec \phi(\vec s) &= (\phi_1(\vec s), \phi_2(\vec s), \dots, \phi_r(\vec s))^{\intercal}\\
\vec \alpha_t &= (\alpha_{1,t}, \alpha_{2,t}, \dots, \alpha_{r, t})^{\intercal}
\end{split}
$${#eq-vecmats}
...
$$\begin{split}
Y(\vec s; t) &= \vec \phi^{\intercal}(\vec s)  \alpha(t)\\
\end{split}
$${#eq-pbvec}


We now want to find the equation defining the evolution of the spectral coefficients, $\vec \alpha_t$.
...
$$\Psi := \int_{\mathcal D_s} \vec \phi(\vec s) \vec \phi(\vec s)^\intercal d\vec s
$${#eq-gram}
...
$$Y(\vec s;t+1) = \vec \phi(\vec s) \alpha(t+1) = \int_{\mathcal D_s} \kappa(\vec s, \vec r) \vec\phi(\vec r)^{\intercal}\vec \alpha(t)d\vec r + \omega_t(\vec s),
$$

We then multiply both sides by $\vec \phi(s)$ and integrate over $\vec s$
...
So, finally, pre-multipling by the inverse of the gram matrix, $\Psi^{-1}$ (@eq-gram), we arrive at the result.
:::
...
Then $\omega_t(\vec s)$ has covariance
...
We write the vector of the process at these points as $\vec Y(t) = (Y(s_{1,t};t), \dots, Y(s_{n_{t},t};t))^\intercal$, and, in it's expanded form $\vec Y_t = \Phi_t \vec\alpha_t$, where $\Phi \in \mathbb R^{r\times n_{t}}$ is
...
$$\begin{split}
\vec Z_t &= \Phi \alpha_t + X_{t} \vec \beta + \vec \epsilon_t, \quad t=0,1,\dots, T,\\
\vec \alpha_{t+1} &= M\vec \alpha_t + \vec\eta_t,\quad t = 1,2,\dots, T,\\
...
Writing $\tilde{\vec{Z}}_t = \vec Z_t - X_t \vec \beta$,

$$\begin{split}
\tilde{\vec Z}_t &= \Phi_{t} \vec \alpha_t + \vec \epsilon_t,\quad &t = 1,2,\dots, T,\\
\vec \alpha_{t+1} &= M \vec \alpha_t + \vec\eta_t,\quad &t = 0,1, \dots, T.\\
\end{split}
$$ {#eq-ldstm}

We should also initialise $\vec \alpha_0 \sim \mathcal N^{r}(\vec m_{0}, \Sigma_{0})$, and fix simple distrubtions to the noise terms,
...
The Kalman filter gives us linear estimates for the distribution of $\vec\alpha_r\mid \{Z_t\}_{t=0,...,r}$ in any dynamical system like @eq-ldstm.
...
$$\begin{split}
\vec m_{t+1\mid t+1} &= \vec m_{t+1\mid t} + K_{t+1} \vec e_{t+1}\\
...
and $\vec e_{t+1}$ are the _prediction errors_

$$\begin{split}
\vec e_{t+1} = \tilde{\vec z}_{t+1}-\Phi_{t+1} \vec m_{t+1\mid t}, \quad t=1,\dots,T 
\end{split}
$$
...
$$\begin{split}
\mathbb V\mathrm{ar}[\vec e_t]=\Sigma_t= \Phi_{t} P_{t\mid t-1} \Phi_{t}^\intercal + \Sigma_\epsilon. 
\end{split}
$$
...
$$\begin{split}
\mathcal L(Z\mid\vec\theta) = -\frac12\sum \log\det(\Sigma_t(\vec\theta)) - \frac12 \sum\vec e_t(\vec\theta)^\intercal\Sigma_{t}(\vec\theta)^{-1} \vec e_t(\vec\theta) - \frac{n_{t}}{2}\log(2*\pi).
\end{split}
$$
...
If a Gaussian distribution has mean $\vec\mu$ and variance matrix $\Sigma$, then the corresponding _information vector_ and _information matrix_ is $\nu = \Sigma^{-1}\mu$ and $Q = \Sigma^{-1}$, correspondingly.
...
\vec\nu_{i\mid j} &= Q_{i\mid j} \vec m_{i\mid j}
\end{split}
$$
...
i_{t} = \Phi_{t}^{\intercal} \Sigma_{\epsilon}^{-1} \vec z_{t}.
\end{split}
$${#eq-obsinfo}
...
Again, we would like to stick with things in the state dimension, so working direclty with the prediction errors $\vec e_t$ should be avoided.
Luckily, by multipliying the errors by $\Phi_t^\intercal \Sigma_\epsilon^{-1}$, we can define the 'information errors' $\vec \iota_t$;

$$\begin{split}
  \vec \iota_t &= \Phi_t^\intercal \Sigma_\epsilon^{-1} \vec e_t = \Phi_t^\intercal \Sigma_\epsilon^{-1} \tilde{\vec z}_t -\Phi_t^\intercal \Sigma_\epsilon^{-1}\Phi_t m_{t\mid t-1}\\
  &= i_t - I_tQ_{t\mid t-1}^{-1}\vec \nu_{t\mid t-1}.
\end{split}
$$
...
$$\begin{split}
  \var[\vec \iota_t] &= \Phi_t^\intercal \Sigma_\epsilon^{-1}\var[\vec e_t]\Sigma_\epsilon^{-1}\Phi_t\\
...
Noting that $\vec \iota$ clearly still has mean zero, this allows us once again to compute the log likelihood, this time through $\vec\iota$


$$\begin{split}
\mathcal L(z_t\mid\vec\theta) = -\frac12\sum \log\det(\Sigma_{\iota, t}(\vec\theta)) - \frac12 \sum\vec \iota_t(\vec\theta)^\intercal\Sigma_{\iota, t}(\vec\theta)^{-1} \vec \iota_t(\vec\theta) - \frac{r}{2}\log(2*\pi).
\end{split}
$$
...
2. Starting from $\vec m_{0\mid 0}=\vec m_0, P_{0\mid0}=\Sigma_0$, run the __Kalman Filter__ to get $\vec m_{t\mid t}$, $P_{t\mid t}$, and $K_t$ for all $t$ @eq-kalman-update,
...
$$\begin{split}
  \vec \nu_{t+1\mid t} &= Q_{t+1\mid t} \vec m_{t+1\mid t}\\
  &= Q_{t+1\mid t} M Q_{t\mid t}^{-1} \vec \nu_{t\mid t} = Q_{t+1\mid t} (M Q_{t\mid t}^{-1}) \vec \nu_{t\mid t}\\
  &= (I-J_t)M^{-\intercal}Q_{t\mid t}M^{-1} (M Q_{t\mid t}^{-1}) \vec \nu_{t\mid t}\\
  &= (I-J_t)M^{-\intercal} \vec \nu_{t\mid t}.
\end{split}
$$
...
Then, writing $\vec m_{t+1\mid t+1}$ in terms of $Q_{t+1\mid t}$ and $\vec \nu_{t+1\mid t}$

$$\begin{split}
  \vec m_{t+1\mid t+1} &= Q_{t+1\mid t}^{-1} \vec \nu_{t+1\mid t} - Q_{t+1\mid t}^{-1}\Phi_{t+1}^\intercal[\Phi_{t+1}Q_{t+1\mid t}^{-1}\Phi_{t+1}^\intercal +\Sigma_\epsilon]^{-1} [\tilde{\vec z}_{t+1} - \Phi_{t+1}Q_{t+1\mid t}^{-1}\vec \nu_{t+1\mit t}]\\
  &= (Q_{t+1\mid t}^{-1} - Q_{t+1\mid t}^{-1}\Phi_{t+1}^\intercal[\Phi_{t+1}Q_{t+1\mid t}^{-1}\Phi_{t+1}^\intercal +\Sigma_\epsilon]^{-1}\Phi_{t+1}Q_{t+1\mid t}^{-1})\vec \nu_{t+1\mid t} \\
  &\quad + Q_{t+1\mid t}^{-1}\Phi_{t+1}^\intercal[\Phi_{t+1}Q_{t+1\mid t}^{-1}\Phi_{t+1}^\intercal +\Sigma_\epsilon]^{-1}\tilde{\vec z}_{t+1}\\
  &= [Q_{t+1\mid t} + I_{t+1}]^{-1}\vec \nu_{t+1\mid t}\\
  &\quad + [Q_{t+1\mid t} + I_{t+1}]^{-1}\Phi_{t+1}\Sigma_\epsilon^{-1}\tilde{\vec z}_{t+1},
\end{split}
$$

and now noting that $\vec\nu_{t+1\mid t+1} = (Q_{t+1\mid t} + I_{t+1}) \vec m_{t+1\mid t+1}$, we complete the proof.
...
$$\begin{split}
  \vec m_{1\mid1} &= (\Phi_1^\intercal \Sigma_\epsilon^{-1} \Phi_1)^{-1} \Phi_1 \Sigma_\epsilon^{-1} \tilde{\vec z}_1,\\
...
$$\begin{split}
  \vec m_{1\mid1} = M \vec m_0 + P_{1\mid0}\Phi_1 [\Phi_1 P_{1\mid0} \Phi_1^\intercal + \Sigma_\epsilon]^{-1}(\tilde{\vec{z}}_1 - \Phi M \vec m_0).\\
\end{split}
$$
...
$$\begin{split}
  \vec m_{1\mid1} &= M \vec m_0 + (P_{1\mid0}^{-1} + \Phi_1^\intercal\Sigma_\epsilon^{-1} \Phi_1)^{-1}\Phi_1^\intercal \Sigma_\epsilon^{-1}(\tilde{\vec{z}}_1 - \Phi_1 M\vec m_0)\\
  &= M \vec m_0 + (\Phi_1^\intercal\Sigma_\epsilon^{-1}\Phi_1)^{-1}\Phi_1^\intercal\Sigma_\epsilon^{-1} \tilde{\vec{z}}_1 - (\Phi_1^\intercal\Sigma_\epsilon^{-1}\Phi_1)^{-1}\Phi_1^\intercal\Sigma_\epsilon^{-1}\Phi_1M\vec m_0\\
  &= (\Phi_1^\intercal\Sigma_\epsilon^{-1}\Phi_1)^{-1}\Phi_1^\intercal\Sigma_\epsilon^{-1} \tilde{\vec{z}}_1.
\end{split}
$$
...
It is worth noting that (@eq-kalmanvague) seems to make a lot of sense; namely, we expect the estimate for $\vec m_0$ to look like a correlated least squares-type estimator like this.
