...
zlocs = data=rand.uniform(keys[0], shape=(100,2))

Y_basis = place_basis(data=zlocs, nres=2)
nbasis = Y_basis.nbasis

G_const = place_basis(nres=1, min_knot_num=1, basis_fun=lambda s, r: 1)
...
s_grid, delta = create_grid(bbox, jnp.array([41,41]))

K_basis = (G_const, G_const, G_const, G_const)
k = (jnp.array([150]), jnp.array([0.002]), jnp.array([-0.1]), jnp.array([0.1]))

alpha0 = jnp.zeros(nbasis).at[64].set(1)

Sigma_eta = sigma2_eta * jnp.eye(nbasis)
Sigma_eps = sigma2_eps * jnp.eye(nobs * T)
Q_eta = jnp.linalg.inv(Sigma_eta)
Q_eps = jnp.linalg.inv(Sigma_eps)
...
# Matches!
print(outer_op(s_grid, s_grid, kernel)[0:4,0:4])
#+end_src
