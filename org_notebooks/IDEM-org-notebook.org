...
frames[0].save(output_path, save_all=True, append_images=frames[1:], duration=gif1.info['duration'], loop=0)

from PIL import Image, ImageSequence

def overlay_gifs(gif1_path, gif2_path, output_path):
    ...
    num_frames_gif2 = len(list(ImageSequence.Iterator(gif2)))
    max_frames = max(num_frames_gif1, num_frames_gif2)
    gif2 = gif2.resize(gif1.size, Image.ANTIALIAS)
