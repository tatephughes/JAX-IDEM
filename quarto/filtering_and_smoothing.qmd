...
Since the information filter function has support for time variying observation shapes, there is a little more work to do.
...
ll2, nus, Qs = fsf.information_filter_indep(nu_0,
                                            Q_0,
                                            M,
                                            PHI_tuple,
                                            sigma2_eta,
...
ms2_df = pd.DataFrame(list(ms), columns = ["x", "y"])
...
obj_grad = jax.grad(objective_kalman)
...
ll, ms3, Ps, _, _, _ = fsf.kalman_filter(m_0,
                                         ...
                                         param_ad['M'],
                                         PHI,
                                         param_ad['sigma2_eta']*jnp.eye(2),
                                         param_ad['sigma2_eps']*jnp.eye(3),
                                         zs.T,)
end_time = time.time()
...
def objective_info(params):
    ...
    Sigma_eps_tuple = tuple([Sigma_eps for _ in range(T)])

    ll, _, _ = fsf.information_filter(nu_0,
                                      Q_0,
                                      M,
                                      PHI_tuple,
                                      Sigma_eta,
...
obj_grad = jax.grad(objective_info)
...
ll, nus, Qs = fsf.information_filter(nu_0,
                                     P_0,
                                     param_ad['M'],
                                     PHI_tuple,
                                     param_ad['sigma2_eta']*jnp.eye(2),
                                     tuple([param_ad['sigma2_eps']*jnp.eye(3) for _ in range(T)]),
                                     zs_tuple,)
