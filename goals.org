...
** Sqinf filter does not support data with missing time points

inf filter does; this is a quirk with the qr decomposition in jax not being quite right when used on empty matrices; it might even be considered a bug. Investigate further.
...
** Why do the filters disagree between preecisions even on small models
** Sort the precision stuff
- Seems to be that PRNG varies across precision. Great.
** Look at more conferences

** Look at GeoPandas for replacing ~st_data~
*** NetCDF?
*** Integrate and mess with some data


** Maybe talk to Andrew Iskauskas about packaging


* 3rd July

** Louis might send some cool papers from bayescomp
** A note on geometric ergodicity and FP roundoff error

** TODO Test more FP things

- Sign up for allstat
- RSS
- BISP

** DONE Start making a filtering presentation
- look at jax smfb for some guidance for git and docs
** TODO Outline structure for a paper



** 10th July

*** use picard for idem? muck around with it
*** Gaussian Quadrature for M construction


* 31st July

Ask for more details for presentation

Work on presentation

Do simpler filter implementations
...
** Presentation
*** Add github and others to final slide
- qr code too
*** change sqrt filters to just talk abou the update


* 4th September

** Implement PKF in JAX-IDEM
** Think more psqrt
** read the bayes linear paper


* TODO Tomorrow (8th Sept)
Tomorrow, i want to have some comparisons between the filters in the sydney radar bayesian inference example page. Now that parallel filtering is fully implemented. I also want to much aboot with various little optimisations to get them all as good as can be.


* 11th September

** Fix kalman jank
- args to solve commands
- maybe nugget
- 0.5(A.T + A)
**** UPDATE: yeah i dont think this is fixable in fp64 even.
- The problem seems to be the scale of $M$ and $\Phi$ in IDEM models. The average eigval of each are about 700 and 16 resp. so $\Sigma_t = \Phi^T (M^T P_{t-1} M + \Sigma_\eta) \Phi + \Sigma_\epsilon$ sees extreme precision issues that arent easily resolved without square-root or inverse methods.
** Read more
*** try and think abt standard kf as bayes linear
** think more abt psqrt
** Take a full day spring cleaning (Monday)
** write up the problem better



* 18th September

- Add assume_a to everything
  - add the satibilised kalman filter
- write up the psqrt problem please
  - maybe also pinf
    - Since now all filters get the same value, and pkal already works in dimension r, I have doubts that this would make any difference.
      - But, like, what if it did tho
        - what if doing the same 'sufficient statistic' thing as in the inf filter lets you store the likelihood in a more 'complete' way?
...
- Fix psqrt
- Send the rischardson paper
- flesh out paper and READ

* 9th October

- ireland https://www.met.ie/climate/available-data/historical-data
- urban observatory

- flesh out subsections
  - look at literature and use that as a reference for the structure

- think about lower dim psqrt

- Print and read all the papers while cuddling dog for emotional support

- Write a good vignette

- german air, get it working idiot

** Vignette/Examples

I want 3 examples

- Sydney Radar as a simple low-T image data set (high n, relatively low r)

- A station-based data set with missing data and potentially low n (German air OR a subset of the Urban Observatory data), with high T

- A bigger image data set with reasonably high T (maybe daily over a month)


* 

- Please support geopandas
- Do the simulation study! work it all out!
  - and prep to send to NCC

* Set alerts in google scholar for papers being published (like sarkka, cressie etc)

- expand literature review
- plug in the smoothers
- read the sqrt parallel kalman filter papers
  - look at simulation smoothers too?

* Link overleaf to github (or just git)
- code up the block gibbs thing and compare to marginal metropolis
- Write up compressed Kalman (and do stabilised and sqrt)


* DONE Make filt_results a proper class with methods to get any filtered quantity (in a lazy way) regardless of the type of filter used. If this is possible in python.
- this is a bad idea, don't do it (though there must be some way of improving this...)



* TODO More stable smoother (and backward sampler)
* TODO Git hygeine >:(
* TODO Posterior for beta
* TODO Finish an initial Gibbs sampler
