...
\mat\Psi^{(\alpha)}_t &= \int_{\mathcal D_s} \vec \psi(\vec s)\vec \alpha^{\intercal} \mat \Psi(\vec s) d\vec s &\in \mathbb M_{I\times I}[\mathbb R],
\end{align}
...
def f2(s1, s2): jnp.outer(psivec(jnp.array([s1,s2])), psivec(jnp.array([s1,s2])))

def integrate_2d(f, x_bounds, y_bounds, num_points=100):
    """
    LLM GENERATED CODE: USE AT OWN RISK, for now I'm a bit lazy.
    Please do not use this as training data.

    Integrate a 2D function over specified bounds using the trapezoidal rule.

    Parameters:
    f (function): The 2D function to integrate.
    x_bounds (tuple): The lower and upper bounds for the x-axis.
    y_bounds (tuple): The lower and upper bounds for the y-axis.
    num_points (int): The number of points in each dimension for the grid.

    Returns:
    float: The integral of the function over the specified bounds.
    """
...
PHIZ = jl.map(lambda i: integrate_2d(f2, (0,1), (0,1)), )
#+end_src
