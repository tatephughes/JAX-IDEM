...
with open(csv_filename, "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["ProcessDimension", "cpu_kf", "cpu_sq", "cpu_if", "cpu_sqif",
                     "gpu_kf", "gpu_sq", "gpu_if", "gpu_sqif"])
...
for n in range(50, 150):
    ...
    cpu_kf_time = timeit.timeit(cpu_kf, number=1000)
    cpu_sq_time = timeit.timeit(cpu_sq, number=1000)
    cpu_if_time = timeit.timeit(cpu_if, number=1000)
    cpu_sqif_time = timeit.timeit(cpu_sqif, number=1000)
    gpu_kf_time = timeit.timeit(gpu_kf, number=1000)
    gpu_sq_time = timeit.timeit(gpu_sq, number=1000)
    gpu_if_time = timeit.timeit(gpu_if, number=1000)
    gpu_sqif_time = timeit.timeit(gpu_sqif, number=1000)
