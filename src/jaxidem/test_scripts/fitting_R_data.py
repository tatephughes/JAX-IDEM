...
df = pd.read_csv(os.path.join(os.path.dirname(__file__), '../../../data/obs_data_r-ide.csv'))
...
const_basis = utilities.place_basis(
    nres=1, min_knot_num=1, basis_fun=lambda s, r: 1)
...
truektrans = (jnp.log(truemodel.kernel.params[0]),
...
trueparams = (jnp.log(truemodel.sigma2_eps), jnp.log(
    truemodel.sigma2_eta), truektrans, truemodel.beta)
...
model1, params = truemodel.fit_kalman_filter(obs_data=obs_data,
                                  X_obs=X_obs,
...
print(f"with likelihood {model1.filter(obs_data, X_obs=X_obs, likelihood='partial')[0].tolist()}")
print(f"\nTrue parameters are: \n{idem.format_params(trueparams)}")
print(f"with likelihood {truemodel.filter(obs_data, X_obs=X_obs, likelihood='partial')[0].tolist()}")
