...
def kalman_filter_indep(
    ...
    def step(carry, z_t):
        ...
        P_oprop = PHI @ P_pred @ PHI.T
        Sigma_t = jnp.fill_diagonal(
            P_oprop, jnp.diag(sigma2_epss) + jnp.diag(P_oprop), inplace=False
        )
...
def sqrt_information_filter_indep(
    ...
    Sigma_eta = jnp.diag(sigma2_etas)
    Sigma_eps_tree = jax.tree.map(jnp.diag, sigma2epss_tree)
    return sqrt_information_filter(
            nu_0,
            R_0,
            M,
            PHI_tree,
...
#@partial(jax.jit, static_argnames=["sigma2_eta_dim", "sigma2_eps_dim", "likelihood"])
def sqrt_information_filter_new(
    nu_0: ArrayLike,
    R_0: ArrayLike,
    M: ArrayLike,
    PHI_tree: tuple,
