...
def sqrt_filter(
    ...
    nbasis = m_0.shape[0]
    nobs = zs.shape[0]
    U_eps = jnp.linalg.cholesky(Sigma_eps)
...
def sqrt_information_filter_iid(
    ...
    def informationify(tup: tuple):
        ...
        R_k = jnp.linalg.qr(PHI_k, mode="r") / jnp.sqrt(sigma2_eps)
        return jnp.vstack((i_k, R_k))
...
def information_filter(
    ...
    if likelihood == "full":
        ...
        lls = jnp.array(
            jax.tree.map(comp_full_likelihood_good,
...
def sqrt_information_filter(
    ...
    def informationify(tup: tuple):
        ...
        PHI_k = tup[1]
        Sigma_eps = tup[2]
        U_eps = jnp.linalg.cholesky(Sigma_eps, upper=False)
        i_k = PHI_k.T @ st(U_eps, st(U_eps.T, z_k, lower=False), lower=True)
        R_k = jnp.linalg.qr(st(U_eps.T, PHI_k, lower=False), mode="r")
        return jnp.vstack((i_k, R_k))
...
def kalman_filter_indep_vd(
    ...
    zs = jnp.array(jax.tree.map(lambda z: jnp.pad(
        z, (0, max_len - z.shape[0]), constant_values=jnp.nan), zs_tree))
    PHIs = jnp.array(jax.tree.map(lambda PHI: jnp.pad(
        PHI, ((0, max_len - PHI.shape[0]), (0, 0)), constant_values=jnp.nan), PHI_tree))
    scan_elts = jnp.concatenate([zs[:, :, None], PHIs], axis=-1)
