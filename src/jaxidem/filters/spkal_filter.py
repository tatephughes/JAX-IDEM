...
def spkal_filter(
    ...
    """
    [UNFINISHED] Building up The Stabilised Temporally Parallel Kalman Filter.

    Currently using this to just check that a few computations are correct, this is not fully stabilised.
    ...
    C_1 = (jnp.eye(r) - D_1 @ PHI_tree[0]) @ P1pred @ (
        jnp.eye(r) - D_1 @ PHI_tree[0]
    ).T + D_1 @ mult_variance(D_1, S2_eps_tree[0], S2_eps_shape)
    ...
    def get_element(mapping_elt: tuple):
        ...
        C_k = mult_variance(
            imdp, S2_eta, S2_eta_shape
        ).T @ imdp.T + D_k @ mult_variance(D_k, S2_eps_k, S2_eps_shape)
