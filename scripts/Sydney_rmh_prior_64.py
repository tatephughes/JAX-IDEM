...
print("This is RMH with a strong prior on shape and scale. (64-bit precision)")
...
radar_df = pd.read_csv('radar_df.csv')
# Censor the data!
...
rmh = blackjax.additive_step_random_walk(lambda flatpars: log_post(unflat(flatpars)), blackjax.mcmc.random_walk.normal(imm/(5.6644/7)))

# Initialize the state
state = rmh.init(init_mean)
...
csv_file = os.path.join(dir, f'results/{current_datetime}_results_rmh_prior_64.csv')



for i in tqdm(range(rmh_n), desc="Sampling... "):

    rmh_key = jax.random.fold_in(sample_key, i)
    state, info = step(rmh_key, state)

    accepted = jnp.asarray(info.is_accepted, dtype=int).reshape((1,))
    rmh_sample.append(jnp.concatenate([accepted, state.position]))
...
#post_mean = jnp.mean(rmh_sample_arr[int(rmh_n/3):,:], axis=0)
#print(post_mean)
...
with open(os.path.join(dir, 'pickles/rmh_sample.pkl'), 'wb') as file:
    pickle.dump((jnp.array(rmh_sample), acc_ratio), file)
print("Done!")
