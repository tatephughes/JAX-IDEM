...
is a dynamics-based spatio-temporal aiming to model diffusion and
...
datasets are almost always high-dimensional, especially when considering
image data (where each pixel might be considered an observation).
Additionally, repeated observations are usually not possible due to
...
A general way of writing a hierarchical (discrete-time) dynamical model
might be
...
\(\{\bv s_i\}_{i=1,\dots, n}\), assume the operators are linear, assert
a Markov condition, and assume the errors are all normal, we get a
...
primary quantities which need estimation, is of dimension \(n\times n\),
of which there are \(T\) matrices to be estimated. Even if we allow the
propagation matrix to be invariant in time, we can still only make
predictions at the stations \(\{\bv s_i\}\).
...
the space, as illustrated in Figure~\ref{fig-examplekernelvar}.

\begin{figure}[h]

\begin{minipage}{0.50\linewidth}

\pandocbounded{\includegraphics[keepaspectratio]{figure/kernel_example_direction_var.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{figure/kernel_example_strength_var.png}}
...
and its spectral counterpart, \(\bv \eta_t\), are. Dewar, Scerri, and
...
\NormalTok{keys }\OperatorTok{=}\NormalTok{ jax.random.split(key, }\DecValTok{3}\NormalTok{)}
...
\NormalTok{T }\OperatorTok{=} \DecValTok{35}
\NormalTok{nobs }\OperatorTok{=} \DecValTok{50}

\NormalTok{coords }\OperatorTok{=}\NormalTok{ jax.random.uniform(}
\NormalTok{                keys[}\DecValTok{1}\NormalTok{],}
\NormalTok{                shape}\OperatorTok{=}\NormalTok{(nobs, }\DecValTok{2}\NormalTok{),}
\NormalTok{                minval}\OperatorTok{=}\DecValTok{0}\NormalTok{,}
\NormalTok{                maxval}\OperatorTok{=}\DecValTok{1}\NormalTok{,}
\NormalTok{            )}

\NormalTok{times }\OperatorTok{=}\NormalTok{ jnp.repeat(jnp.arange(}\DecValTok{1}\NormalTok{, T }\OperatorTok{+} \DecValTok{1}\NormalTok{), coords.shape[}\DecValTok{0}\NormalTok{])}
\NormalTok{rep\_coords }\OperatorTok{=}\NormalTok{ jnp.tile(coords, (T, }\DecValTok{1}\NormalTok{))}
\NormalTok{x }\OperatorTok{=}\NormalTok{ rep\_coords[:,}\DecValTok{0}\NormalTok{]}
\NormalTok{y }\OperatorTok{=}\NormalTok{ rep\_coords[:,}\DecValTok{1}\NormalTok{]}

\NormalTok{process\_data, obs\_data }\OperatorTok{=}\NormalTok{ model.simulate(keys[}\DecValTok{2}\NormalTok{], x, y, times)}
\end{Highlighting}
\end{Shaded}
...
storing all data as JAX arrays.

The optional \texttt{plotters} module contains some functions to produce
plotly plots of data in this format;

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# SAVING GIFS IS SLOW, so this doesn\textquotesingle{}t eval on every run.}

\CommentTok{\#process\_data.save\_plot(\textquotesingle{}figure/process\_data\_example.png\textquotesingle{})}
\CommentTok{\#obs\_data.save\_plot(\textquotesingle{}figure/obs\_data\_example.png\textquotesingle{})}

\ImportTok{import}\NormalTok{ jaxidem.plotters }\ImportTok{as}\NormalTok{ plotters}

\NormalTok{obs\_frames }\OperatorTok{=}\NormalTok{ plotters.plotly\_st\_scatter(obs\_data, title}\OperatorTok{=}\StringTok{"Simple IDEM process"}\NormalTok{)}
\NormalTok{obs\_fig }\OperatorTok{=}\NormalTok{ plotters.make\_st\_fig(obs\_frames, spacing}\OperatorTok{=}\NormalTok{[}\FloatTok{0.05}\NormalTok{, }\FloatTok{0.08}\NormalTok{])}

\NormalTok{plotters.apply\_catppuccin\_mocha(obs\_fig, fontsize }\OperatorTok{=} \DecValTok{20}\NormalTok{)}

\NormalTok{obs\_fig.update\_xaxes(title\_text}\OperatorTok{=}\VariableTok{None}\NormalTok{, showticklabels}\OperatorTok{=}\VariableTok{False}\NormalTok{)}
\NormalTok{obs\_fig.update\_yaxes(title\_text}\OperatorTok{=}\VariableTok{None}\NormalTok{, showticklabels}\OperatorTok{=}\VariableTok{False}\NormalTok{)}

\NormalTok{obs\_fig.update\_layout(}
\NormalTok{    xaxis}\OperatorTok{=}\BuiltInTok{dict}\NormalTok{(showticklabels}\OperatorTok{=}\VariableTok{False}\NormalTok{),}
\NormalTok{    yaxis}\OperatorTok{=}\BuiltInTok{dict}\NormalTok{(showticklabels}\OperatorTok{=}\VariableTok{False}\NormalTok{, scaleanchor}\OperatorTok{=}\StringTok{"x"}\NormalTok{)}
\NormalTok{)}

\NormalTok{proc\_frames }\OperatorTok{=}\NormalTok{ plotters.plotly\_st\_grid(process\_data)}
\NormalTok{proc\_fig }\OperatorTok{=}\NormalTok{ plotters.make\_st\_fig(proc\_frames, spacing}\OperatorTok{=}\NormalTok{[}\FloatTok{0.05}\NormalTok{, }\FloatTok{0.08}\NormalTok{])}

\NormalTok{plotters.apply\_catppuccin\_mocha(proc\_fig, fontsize }\OperatorTok{=} \DecValTok{20}\NormalTok{)}

\NormalTok{proc\_fig.update\_xaxes(title\_text}\OperatorTok{=}\VariableTok{None}\NormalTok{, showticklabels}\OperatorTok{=}\VariableTok{False}\NormalTok{)}
\NormalTok{proc\_fig.update\_yaxes(title\_text}\OperatorTok{=}\VariableTok{None}\NormalTok{, showticklabels}\OperatorTok{=}\VariableTok{False}\NormalTok{)}

\NormalTok{proc\_fig.update\_layout(}
\NormalTok{    xaxis}\OperatorTok{=}\BuiltInTok{dict}\NormalTok{(showticklabels}\OperatorTok{=}\VariableTok{False}\NormalTok{),}
\NormalTok{    yaxis}\OperatorTok{=}\BuiltInTok{dict}\NormalTok{(showticklabels}\OperatorTok{=}\VariableTok{False}\NormalTok{, scaleanchor}\OperatorTok{=}\StringTok{"x"}\NormalTok{)}
\NormalTok{)}

\CommentTok{\#obs\_fig.write\_image("figure/process\_data\_example.png", format="png", scale=2)}
\CommentTok{\#proc\_fig.write\_image("figure/obs\_data\_example.png", format="png", scale=2)}


\CommentTok{\#plotters.save\_st\_gif(proc\_frames, "figure/process\_data\_example.gif", apply\_theme=plotters.apply\_catppuccin\_mocha)}
\CommentTok{\#plotters.save\_st\_gif(obs\_frames, "figure/obs\_data\_example.gif", apply\_theme=plotters.apply\_catppuccin\_mocha)}
\end{Highlighting}
\end{Shaded}
...
\pandocbounded{\includegraphics[keepaspectratio]{figure/process_data_example.gif}}
...
\pandocbounded{\includegraphics[keepaspectratio]{figure/obs_data_example.gif}}
...
Since its initial formulation in the 50s by a variety of authors (Kálmán
included) there have been many variations of the Kalman filter proposed,
even as recently as this decade with the temporally paralellised Kalman
filter, more technically a variant of the information form of the Kalman
filter, by Särkkä and Garcı́a-Fernández (2020).
...
information filter) can encounter numerical stability issues. Note the
...
still guaranteed to be positive (semi-)definite, when done in floating
point arithmetic (especially in single-precision or lower), the result
can often be numerically indefinite. When the variances are very low (as
...
such a computation involves a Cholesky decomposition). Even if such is
...
The square-root Kalman filter has its origins soon after the standard
...
(and, more recently, its parallel counterpart, to be covered in section
{[}TBD{]}) usually suffice.
...
The update step for the variance is a bit more involved, since it is not
clear how to get a square root from (Equation~\ref{eq-kalman-update}).
To do this, we re-write the variance in \emph{Joseph Stabilised Form} as
follows;

\begin{lemma}[]\protect\hypertarget{lem-stable_form}{}\label{lem-stable_form}
...
often called the \emph{Joseph Stabilised Form}.

\end{lemma}

\begin{proof}
As in (Chin 2023), write

\[\begin{split}
  \bv \alpha_t - \bv m_{t+1\mid t+1} & = \bv \alpha_t - \bv m_{t \mid t+1} - K_{t+1} (\tilde{\bv z}_{t+1} - \Phi_{t+1} \bv m_{t+1\mid t})\\
  &= \bv \alpha_t - \bv m_{t \mid t+1} - K_{t+1} (\Phi_{t+1} \bv m_{t+1} + \bv \epsilon_{t+1} - \Phi_{t+1}\bv m_{t+1\mid t})\\
...
Which, noting that \(\bv \epsilon_{t+1}\) are independant of
\(\bv \alpha_t\) and
\(\mathbb E[(\bv \alpha_t - \bv m_{t+1 \mid t})^\intercal (\bv \alpha_t - \bv m_{t+1 \mid t})]=P_{t+1\mid t}\),
gets us the result.

\qed
\end{proof}

Now this is in a form that allows us to easily find the square root with
the QR trick;
...
\section{The Temporally Parallel
Filters}\label{the-temporally-parallel-filters}

As mentioned before, the Kalman filter, and all its variants, use a scan
operation. This seems like not a trivially parallelisable operation; it
seems very sequential in nature. However, if the operation being used
within the scan is \emph{accociative}, then there is an algorithm that
allows us to partially parallelise such an algorithm. The Kalman Filter,
on it's own, is clearly not associative; for a start, its output type is
not the same as its input type, so it can't be. But can we re-write it
so that it is? This is what \cite{sarkka2020temporal} recently did.

We would like our elements to of the same type as our outputs, and for
these to be able to be computed in parallel. A good start might be to
consider `partial posteriors' containing only some of the data;

\begin{equation}\phantomsection\label{eq-ass-elt}{\begin{split}
  p(\bv\alpha_t \mid \bv\alpha_{t-1}, \bv z_{t-l:t}),
\end{split}
}\end{equation}

that is, posteriors containing possibly limited number of previous
elements. The idea then would be that it is easy to compute

\[\begin{split}
  p(\bv\alpha_t \mid \bv\alpha_{t-1}, \bv z_t),
\end{split}
\]

for each time \(t=1,\dots, t\). The, if we can combine two such
densities on non-overlpping times, we `add the data together' to get the
posterior at the latest time possible, with all the data available in
the two elements, combined; writing this using a prospective operator
\(\oplus\),

\[\begin{split}
  p(\bv\alpha_t \mid \bv\alpha_{t-1}, \bv z_{t-l:t}) \oplus p(\bv\alpha_{t+k} \mid \bv\alpha_{t+k-1}, \bv z_{t:t+k}) = p(\bv\alpha_{t+k} \mid \bv\alpha_{t+k-1}, \bv z_{t-l:t+k}).
\end{split}
\]

\section{Smoothing}\label{smoothing}

(Largely unfinished. Focus has been on filtering, as that is what gives
the likelihood as an output.)
...
Very similar formulas can be found for square-root and information
variations.
...
Chin, Toshio Michael. 2023. {``Square-Root Formulas for Kalman Filter,
Information Filter, and RTS Smoother: Links via Boomerang Prediction
Residual.''} \emph{IPN Progress Report} 42: 233.
