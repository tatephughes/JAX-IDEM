...
roots of variances \footnote{A matrix \(A\) is said to be a `square
  ...
  the QR decomposition (or LQ decomposition), which effectively computes
...
Taking the QR decomposition of the verticle block yields QR, and since
...
roots of the information matrices. For convenience, we actually carry
through the \emph{lower triangular} square roots of the information
matrices. As above, any upper triangular matrix will be represented with
a \(U\), and any lower triangular matrix with an \(L\). Most
implementations of the Cholesky decompositions allow you to work with
either.

We now carry the data's information matrix's (Equation~\ref{eq-obsinfo})
square root as well,
\(L_t^{(I)} = \sqrt(\Phi_t^\intercal\Sigma_\epsilon \Phi_t)\), withthe
same observation vector.

So, once again, beginning with the lower-triangular cholesky
decomposition \(Q_0 = L^\intercal L\), and the upper-triangular
\(\Sigma_{\eta} = U_{\eta}^\intercal U_{\eta}\) and
