...
The Integro-Difference equation model (here abbreviated as IDEM ^[Historically, this has been abbreviated as IDE. However, with that abbreviation almost universally meaning 'Integrated Development Environment', here, we choose to include the 'M' in the abbreviation.]) is a dynamics-based spatio-temporal aiming to model diffusion and convection by making the value of a process a weighted average of it's previous time, plus noise.

As common and widespread as the problem is, spatio-temporal modelling still presents a great deal of difficulty. Inherently, spatio-temporal datasets are almost always high-dimensional, especially when considering image data (where each pixel might be considered an observation). 
Additionally, repeated observations are usually not possible due to their time dependence. 
...
A general way of writing a hierarchical (discrete-time) dynamical model might be
...
If we discretize the space into $n$ spatial locations $\{\bv s_i\}_{i=1,\dots, n}$, assume the operators are linear, assert a Markov condition, and assume the errors are all normal, we get a simple linear dynamic system;
...
However, this model is restrictive and high-dimensional; $M_t$, the primary quantities which need estimation, is of dimension $n\times n$, of which there are $T$ matrices to be estimated. 
...
This creates a 'spatially variant kernel', where the direction of flow varies across the space, as illustrated in @fig-examplekernelvar.
...
k = (
    jnp.array([150]),
    jnp.array([0.2]),
    0.1 * jax.random.normal(keys[0], shape=(K_basis[2].nbasis,)),
    0.1 * jax.random.normal(keys[1], shape=(K_basis[3].nbasis,)),
)
kernel = idem.param_exp_kernel(K_basis, k)


kernel_fig = plotters.kernel_plot(kernel)

plotters.apply_catppuccin_mocha(kernel_fig, fontsize=20)
kernel_fig.update_layout(
    xaxis=dict(showticklabels=False, range=[0,1]),
    yaxis=dict(showticklabels=False, range=[0,1], scaleanchor="x")
)

kernel_fig.write_image("figure/kernel_example_direction_var.png", format="png", scale=2)
...
z_grid = jnp.full((ny, nx), jnp.nan, dtype=float)
z_grid = z_grid.at[yi, xi].set(z)


strength_fig = go.Figure(data=go.Heatmap(z=z_grid,
                                         x=ux,
                                         y=uy,
                                         colorscale="Viridis",
                                         showscale = True)
                         )
plotters.apply_catppuccin_mocha(strength_fig, 20)
...
offset_at_centre = offset(centre)

arrow = go.layout.Annotation(dict(
                    x=centre[0]-offset_at_centre[0],
                    y=centre[1]-offset_at_centre[1],
                    xref="x", yref="y",
                    text="",
                    showarrow=True,
                    axref="x", ayref='y',
                    ...
                    arrowhead=3,
                    arrowwidth=5,
                    arrowcolor=plotters.cat['blue'],)
                )

strength_fig.add_trace(go.Scatter(x=[0.5],
                                  y=[0.5],
                                  mode = 'markers',
                                  marker_symbol = 'cross',
                                  marker_size = 30))

strength_fig.update_layout(annotations=[arrow])

strength_fig.write_image("figure/kernel_example_strength_var.png", format="png", scale=2)
...
We still have to set out what the process noise, $\omega_t(\bv s)$, and its spectral counterpart, $\bv \eta_t$, are. 
...
M &= \left[\int_{\mathcal D_s}\bv\phi(\bv s) \bv\phi(\bv s)^\intercal d\bv s\right]^{-1} \int_{\mathcal D_s^2}\bv\phi(\bv s) \kappa(\bv s, \bv r; \bv\theta_\kappa)\bv\phi(\bv r)^\intercal d\bv r d \bv s,
\end{split}
$$
...
The optional ```plotters``` module contains some functions to produce plotly plots of data in this format;

```{python}
#| eval: false
#| output: false

# SAVING GIFS IS SLOW, so this doesn't eval on every run.
...
#obs_fig.write_image("figure/process_data_example.png", format="png", scale=2)
#proc_fig.write_image("figure/obs_data_example.png", format="png", scale=2)


#plotters.save_st_gif(proc_frames, "figure/process_data_example.gif", apply_theme=plotters.apply_catppuccin_mocha)
#plotters.save_st_gif(obs_frames, "figure/obs_data_example.gif", apply_theme=plotters.apply_catppuccin_mocha)
...
![Process Simulation](figure/process_data_example.gif)

![Observation Simulation](figure/obs_data_example.gif)
...
Note the update step in the Kalman filter;
...
While analytically, the result is still guaranteed to be positive (semi-)definite, when done in floating point arithmetic (especially in single-precision or lower), the result can often be numerically indefinite.
...
When this happens, computation of the likelihood likely fails (certainly when such a computation involves a Cholesky decomposition).
...
The square-root Kalman filter has its origins soon after the standard Kalman filter gained popularity [@kaminski1971discrete].
Of course, computational and memory constraints necessitated stable and memory-efficient approaches, while today the standard Kalman filter (and, more recently, its parallel counterpart, to be covered in section [TBD]) usually suffice.
...
The update step for the variance is a bit more involved, since it is not clear how to get a square root from (@eq-kalman-update).
To do this, we re-write the variance in _Joseph Stabilised Form_ as follows;


::: {#lem-stable_form}

The update step of the Kalman filter can be written

$$\begin{split}
  P_{t+1\mid t+1}= (I - K_{t+1} \Phi_{t+1}) P_{t+1\mid t}(I - K_{t+1} \Phi_{t+1})^\intercal + K_{t+1}\Sigma_{\epsilon}K_{t+1}^{\intercal},
\end{split}
$$

often called the _Joseph Stabilised Form_.

:::

::: {.proof}
As in [@chin2023], write

$$\begin{split}
  \bv \alpha_t - \bv m_{t+1\mid t+1} & = \bv \alpha_t - \bv m_{t \mid t+1} - K_{t+1} (\tilde{\bv z}_{t+1} - \Phi_{t+1} \bv m_{t+1\mid t})\\
  &= \bv \alpha_t - \bv m_{t \mid t+1} - K_{t+1} (\Phi_{t+1} \bv \alpha_{t} + \bv \epsilon_{t+1} - \Phi_{t+1}\bv m_{t+1\mid t})\\
  &= (I - K_{t+1} \Phi_{t+1})(\bv \alpha_t - \bv m_{t+1 \mid t}) - K_{t+1}\bv \epsilon_{t+1}.
\end{split}
$$
...
$$\begin{split}
  P_{t+1\mid t+1} &= \mathbb C\mathrm{ov}[\bv \alpha_t]\\
  &= \mathbb E[(\bv \alpha_t - \bv m_{t+1\mid t+1})^\intercal(\bv \alpha_t - \bv m_{t+1\mid t+1})]\\
\end{split}
$$

and $\bv \epsilon_{t+1}$ are independant of $\bv \alpha_t$, we get the result.
...
Now this is in a form that allows us to easily find the square root with the QR trick;
...
## The Temporally Parallel Filters

As mentioned before, the Kalman filter, and all its variants, use a scan operation.
This seems like not a trivially parallelisable operation; it seems very sequential in nature.
However, if the operation being used within the scan is _accociative_, then there is an algorithm that allows us to partially parallelise such an algorithm.
The Kalman Filter, on it's own, is clearly not associative; for a start, its output type is not the same as its input type, so it can't be.
But can we re-write it so that it is?
This is what \cite{sarkka2020temporal} did.

We would like our elements to of the same type as our outputs, and for these to be able to be computed in parallel.
A good start might be to consider 'partial posteriors' containing only some of the data;

$$\begin{split}
  p(\bv\alpha_k \mid \bv\alpha_{k-i-l}, \bv z_{k-i:k}),
\end{split}
$$ {#eq-ass-elt}

that is, posteriors containing possibly limited number of previous elements.
The idea then would be that it is easy to compute

$$\begin{split}
  p(\bv\alpha_k \mid \bv\alpha_{k-1}, \bv z_k),
\end{split}
$$

for each time $k=1,\dots, T$. 
Then, if we can combine two such densities on non-overlapping times, we 'add the data together' to get the posterior at the latest time possible, with all the data available in the two elements, combined; writing this using a prospective operator $\oplus$,

$$\begin{split}
  p(\bv\alpha_k \mid \bv\alpha_{k-i-1}, \bv z_{k-i:k}) &\oplus p(\bv\alpha_{k+j} \mid \bv\alpha_{k}, \bv z_{k+1:k+j}) \\
  &= p(\bv\alpha_{k+j} \mid \bv\alpha_{k-i-1}, \bv z_{k-i:k+j}).
\end{split}
$$

However, considering the equation needed to get from the RHS to the LHS; 

$$\begin{split}
  &p(\bv\alpha_{k+j} \mid \bv\alpha_{k-i-1}, \bv z_{k-i:k+j}) \\
  &= \frac{
    p(\bv z_{k+1:k}, \bv\alpha_{k+j}\mid \bv\alpha_{k-i-1}, \bv z_{k-i:k})
  }{
    p(\bv z_{k+1:k+j}\mid \bv \alpha_{k-i-1}, \bv z_{k-i:k})
  }\\
  &= \frac{
    \int p(\bv \alpha_k, \bv \alpha_{k+j} ,\bv z_{k+1:k+j} \mid \bv \alpha_{k-i-1}, \bv z_{k-i:k}) d \bv \alpha_{k-i-1}
  }{
    \int p(\bv \alpha_k, \bv z_{k+1:k+j} \mid \bv \alpha_{k-i-1}\bv z_{k+1:k+j}) d \bv \alpha_{k-i-1}
  }\\
  &= \frac{
    \int p(\bv \alpha_k \mid \bv \alpha_{k-i-1}, \bv z_{k-i}) p(\bv \alpha_{k+j}, \bv z_{k+1:k+j} \mid \bv \alpha_{k}) d \bv \alpha_{k-i-1}
  }{
    \int p(\bv z_{k+1:k+j} \mid \bv \alpha_k)p(\bv \alpha_k \mid \bv \alpha_{k}, \bv z_{k+1:k+j}) d \bv \alpha_{k-i-1}
  }\\
  &= \frac{
    \int p(\bv z_{k+1:k+j} \mid \bv \alpha_{k}) p(\bv \alpha_k \mid \bv \alpha_{k-i-1}, \bv z_{k-i:k}) p(\bv \alpha_{k+j} \mid \bv\alpha_{k}, \bv z_{k+1:k+j}) d \bv \alpha_{k}
  }{
    \int p(\bv z_{k+1:k+j} \mid \bv \alpha_{k}) p(\bv \alpha_k \mid \bv \alpha_{k-i-1}, \bv z_{k-i:k}) d \bv \alpha_{k-i-1}.
  }
\end{split} 
$$ {#eq-tppost}

We see that we need the 'likelihood' term $p(\bv z_{k+1:k+j} \mid \bv \alpha_{k})$ to perform the 'update'. 
We can simply consider this as a part of our associative alements, leading to the elements

$$\begin{split}
  a_{k+1:k+j} := \left(\begin{matrix}
      p(\bv \alpha_{k+j} \mid \bv \alpha_{k}, \bv z_{k+1:k+j})\\
      p(\bv z_{k+1:k+j} \mid \bv \alpha_{k})
    \end{matrix}\right).
\end{split}
$$

Now, if we can find the operation that gives $a_{k-i:k}\oplus a_{k+1:k+j} = a_{k-i:k+j}$, and prove that that operation is associative, then we can use an associative scan to compute the filtered distributions in a temporally parallel way.
We defer the proof of associativity of this operation to \cite{sarkka2020temporal}.
They also then find a similar relation to @eq-tppost for the likelihood, then use some convenient Normal-density identities to derive the update formulas.

However, here, it is informative to write it using other more natural tools, in order to find a stabilised form of the filter, akin to the Joseph form referenced previously.


 
## Smoothing

(Largely unfinished. Focus has been on filtering, as that is what gives the likelihood as an output.)
...
Very similar formulas can be found for square-root and information variations.
